...
# 加载君君专用的环境变量
env_path_junjun = Path(__file__).parent / ".env.junjun"
env_path_default = Path(__file__).parent / ".env"

if env_path_junjun.exists():
    load_dotenv(str(env_path_junjun), override=True)
    print(f"✓ 已加载君君专用配置: .env.junjun")
elif env_path_default.exists():
    load_dotenv(str(env_path_default), override=True)
    print(f"✓ 已加载默认配置: .env")
else:
    print("⚠ 未找到配置文件")
