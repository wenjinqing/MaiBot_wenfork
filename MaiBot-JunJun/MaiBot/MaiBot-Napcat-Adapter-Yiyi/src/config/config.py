...
def update_config():
    ...
    # 读取旧配置文件和模板文件（兼容 BOM / 前导空白）
    with open(old_config_path, "r", encoding="utf-8") as f:
        old_text = f.read().lstrip("\ufeff \t\r\n")
        old_config = tomlkit.parse(old_text)
    with open(template_path, "r", encoding="utf-8") as f:
        new_text = f.read().lstrip("\ufeff \t\r\n")
        new_config = tomlkit.parse(new_text)
...
def load_config(config_path: str) -> Config:
    ...
    with open(config_path, "r", encoding="utf-8") as f:
        text = f.read().lstrip("\ufeff \t\r\n")
        config_data = tomlkit.parse(text)
...
# 尝试更新配置；如果失败则退回到直接加载当前配置文件
try:
    update_config()
except Exception as e:  # 防止配置迁移异常导致程序无法启动
    logger.critical(f"更新配置文件失败，将跳过迁移并直接加载当前 config.toml: {e}")
