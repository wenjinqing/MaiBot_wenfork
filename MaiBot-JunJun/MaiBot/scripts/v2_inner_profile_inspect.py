...
def main() -> int:
    ...
    if gate or inj or side:
        if native and not side:
            planner_llm = True  # native gate still one LLM when gate path runs
        else:
            planner_llm = True  # full legacy plan or side needs full planner
    print(
