...
class ChatStreams(BaseModel):
    ...
    # 多机器人支持
    bot_id = TextField(default="maimai_main", index=True)  # 机器人ID
    ...
    class Meta:
        ...
        indexes = (
            # 复合索引用于按机器人和stream_id查询
            (('bot_id', 'stream_id'), False),
        )
...
class LLMUsage(BaseModel):
    ...
    model_assign_name = TextField(null=True, index=True)  # 添加索引
    model_api_provider = TextField(null=True, index=True)  # 添加索引
    ...
    class Meta:
        ...
        indexes = (
            # 复合索引用于统计查询
            (('user_id', 'timestamp'), False),  # 按用户和时间统计
            (('model_name', 'timestamp'), False),  # 按模型和时间统计
        )
...
class Messages(BaseModel):
    ...
    time = DoubleField(index=True)  # 消息时间戳 - 添加索引用于时间范围查询
    ...
    chat_info_stream_id = TextField(index=True)  # 添加索引
    chat_info_platform = TextField()
    chat_info_user_platform = TextField()
    chat_info_user_id = TextField(index=True)  # 添加索引
    ...
    user_id = TextField(null=True, index=True)  # 添加索引
    ...
    class Meta:
        ...
        indexes = (
            # 复合索引用于常见查询模式
            (('chat_id', 'time'), False),  # 按聊天ID和时间查询
            (('user_id', 'time'), False),  # 按用户ID和时间查询
        )
...
class ActionRecords(BaseModel):
    ...
    time = DoubleField(index=True)  # 消息时间戳 - 添加索引
...
class PersonInfo(BaseModel):
    ...
    person_name = TextField(null=True, index=True)  # 个人名称 (允许为空) - 添加索引
    name_reason = TextField(null=True)  # 名称设定的原因
    platform = TextField(index=True)  # 平台 - 添加索引
    ...
    # 亲密度相关字段
    relationship_value = FloatField(default=0.0, index=True)  # 亲密度值 (0-100，浮点数)
    last_chat_time = FloatField(null=True, index=True)  # 最后聊天时间戳
    total_messages = IntegerField(default=0)  # 总消息数
    chat_frequency = FloatField(default=0.0)  # 聊天频率评分 (0-10)

    # 扩展字段
    mood_value = IntegerField(default=50)  # 当前心情值 (0-100，50为中性)
    user_avatar = TextField(null=True)  # 用户头像URL
    special_tags = TextField(null=True)  # 特殊标签 (JSON格式，如 ["好友", "熟人", "话痨"])
    first_meet_time = FloatField(null=True)  # 首次见面时间戳
    interaction_score = FloatField(default=0.0)  # 互动评分 (综合评分)

    # 恋爱系统字段
    is_in_love = BooleanField(default=False)  # 是否处于恋爱状态
    confession_time = FloatField(null=True)  # 表白时间戳
    love_response = TextField(null=True)  # 用户对表白的回应 (accepted/rejected/thinking)
    anniversary_date = FloatField(null=True)  # 纪念日（确认关系的日期）
    ...
    class Meta:
        ...
        indexes = (
            # 复合索引用于按平台和用户ID查询
            (('platform', 'user_id'), False),
            # 复合索引用于按机器人、平台和用户ID查询
...
class Expression(BaseModel):
    ...
    class Meta:
        ...
        indexes = (
            # 复合索引用于按机器人和chat_id查询
            (('bot_id', 'chat_id'), False),
        )
...
class ThinkingBack(BaseModel):
    ...
    found_answer = BooleanField(default=False, index=True)  # 是否找到答案 - 添加索引
    answer = TextField(null=True)  # 答案内容
    thinking_steps = TextField(null=True)  # 思考步骤（JSON格式）
    create_time = DoubleField()  # 创建时间
    update_time = DoubleField(index=True)  # 更新时间 - 添加索引
...
class RelationshipHistory(BaseModel):
    ...
    event_type = TextField(index=True)  # 事件类型
    # 事件类型：
    # - level_up: 关系等级提升
    # - level_down: 关系等级下降
    # - confession: 表白
    # - confession_accepted: 表白被接受
    # - confession_rejected: 表白被拒绝
    # - breakup: 分手
    # - penalty: 惩罚
    # - decay: 衰减
    # - mood_change: 心情变化
    ...
    old_value = FloatField(null=True)  # 旧值（亲密度/心情值）
    new_value = FloatField(null=True)  # 新值
    change_amount = FloatField(null=True)  # 变化量
    reason = TextField(null=True)  # 原因/描述
    details = TextField(null=True)  # 详细信息（JSON格式）
    timestamp = DoubleField(index=True)  # 时间戳

    class Meta:
        table_name = "relationship_history"
        indexes = (
            # 复合索引用于按用户和时间查询
            (('user_id', 'platform', 'timestamp'), False),
            (('event_type', 'timestamp'), False),
            # 复合索引用于按机器人、用户和平台查询
            (('bot_id', 'user_id', 'platform'), False),
        )
...
class ReminderTasks(BaseModel):
    ...
    task_id = TextField(primary_key=True)  # 任务ID
    person_id = TextField(index=True)  # 用户的person_id
    stream_id = TextField(index=True)  # 聊天流ID
    task_content = TextField()  # 任务内容（如"出门"）
    remind_time = DoubleField(index=True)  # 提醒时间戳
    created_time = DoubleField()  # 创建时间
    is_completed = BooleanField(default=False, index=True)  # 是否已完成
    is_cancelled = BooleanField(default=False)  # 是否已取消
    repeat_type = TextField(null=True)  # 重复类型（daily/weekly/monthly/null）
    repeat_count = IntegerField(default=0)  # 已重复次数

    class Meta:
        table_name = "reminder_tasks"
        indexes = (
            # 复合索引用于查询待执行任务
            (('remind_time', 'is_completed', 'is_cancelled'), False),
        )
