...
class MemoryConfig(ConfigBase):
    ...
    """是否启用详细的用户记忆摘要（包括聊过的话题、有趣的记忆点等）"""

    memory_days_limit: int = 30
    """记忆检索的时间范围（天数），默认30天"""
...
@dataclass
class ProactiveChatConfig(ConfigBase):
    """主动对话配置类"""
    ...
    """最少空闲多久才主动发起（分钟），默认60分钟"""

    max_daily_proactive: int = 3
    """每个聊天流每天最多主动几次，默认3次"""

    silent_hours: str = "23:00-07:00"
    """静默时间段，在此时间段内不主动发起对话"""

    enable_in_groups: bool = True
    """是否在群聊中启用主动对话"""

    enable_in_private: bool = True
    """是否在私聊中启用主动对话"""


@dataclass
class ReminderConfig(ConfigBase):
    """提醒任务配置类"""
...
class RepeatConfig(ConfigBase):
    ...
    threshold: int = 4
    """复读阈值，连续几条相同消息时触发复读，默认4条"""

    min_interval_seconds: int = 60
    """最小复读间隔（秒），避免频繁复读，默认60秒"""

    min_message_length: int = 1
    """最小消息长度，太短的消息不复读，默认1个字符"""

    max_message_length: int = 50
    """最大消息长度，太长的消息不复读，默认50个字符"""


@dataclass
class BotInstanceConfig(ConfigBase):
    """单个机器人实例的完整配置"""
    ...
    bot: BotConfig = field(default_factory=lambda: BotConfig(platform="qq", qq_account="", nickname=""))
    """机器人基础配置"""
