...
class ChatHistorySummarizer:
    ...
    def __init__(self, chat_id: str, check_interval: int = 60):
        ...
        self.impression_llm = LLMRequest(
            model_set=model_config.model_task_config.utils_small, request_type="chat_history.impression"
        )
    ...
    async def _package_and_store(self):
        ...
        try:
            ...
            # 提取并更新参与者的用户印象
            asyncio.create_task(self._extract_and_update_impressions(messages, original_text, summary))
...
请直接返回JSON，不要包含其他内容。"""
    ...
    async def _extract_and_update_impressions(
        self,
        messages: List[DatabaseMessages],
        original_text: str,
        summary: str,
        ...
        try:
            ...
            # 收集非bot参与者
            participants_map: dict = {}  # person_id -> (platform, user_id, person_name)
            for msg in messages:
                if msg.user_info.user_id == bot_user_id:
                    continue
                platform = (
                ...
                if person.is_known and person.person_id not in participants_map:
                    participants_map[person.person_id] = (platform, user_id, person.person_name or user_id)

            if not participants_map:
                return

            for person_id, (platform, user_id, person_name) in participants_map.items():
                try:
                    await self._extract_impression_for_person(
                        person_id=person_id,
                        platform=platform,
                        user_id=user_id,
                    ...
                    logger.error(f"{self.log_prefix} 提取用户 {person_name} 印象时出错: {e}")

        except Exception as e:
            logger.error(f"{self.log_prefix} 提取用户印象时出错: {e}")

    async def _extract_impression_for_person(
        self,
        person_id: str,
        platform: str,
        user_id: str,
        ...
        """为单个用户提取印象并追加到 memory_points"""
        bot_name = global_config.bot.nickname

        prompt = f"""你是{bot_name}，请根据以下聊天记录，提取你对用户"{person_name}"的新印象。
...
【任务】
从聊天中提取关于"{person_name}"的具体印象，包括：
- 性格特点（如：开朗、内向、幽默、认真等）
- 兴趣爱好（如：喜欢游戏、动漫、音乐等）
- 行为习惯（如：夜猫子、话多、喜欢发表情等）
- 说话风格（如：喜欢用网络用语、说话简短等）
- 其他值得记住的特点

【要求】
1. 只提取在本次聊天中能观察到的信息，不要推测
2. 每条印象用一句话描述，简洁具体
3. 如果没有观察到有价值的信息，返回空列表
4. 以JSON格式返回，格式如下：
{{
    "impressions": [
        {{"category": "性格", "content": "说话风趣幽默，经常开玩笑"}},
        {{"category": "兴趣", "content": "喜欢玩FPS游戏，提到了《三角洲行动》"}}
    ]
}}
...
直接返回JSON，不要包含其他内容："""
        ...
        try:
            response, _ = await self.impression_llm.generate_response_async(
                prompt=prompt,
                temperature=0.3,
                max_tokens=400,
            )
            ...
            for imp in impressions:
                category = imp.get("category", "其他").strip()
                ...
                # 检查是否已有相似内容（简单包含检查）
                is_dup = any(
                    content in existing or existing.split(":", 2)[1].strip() in content
                    for existing in person.memory_points
                    if existing and isinstance(existing, str) and len(existing.split(":", 2)) >= 2
                )
                if not is_dup:
                    person.memory_points.append(f"{category}:{content}:1.0")
                    added += 1

            if added > 0:
                person.sync_to_database()
                logger.info(f"{self.log_prefix} 为用户 {person_name} 新增 {added} 条印象")

        except Exception as e:
            logger.error(f"{self.log_prefix} 解析印象提取结果失败: {e}")
