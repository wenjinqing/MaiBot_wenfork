...
class ChatBot:
    ...
    async def message_process(self, message_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 复读检测已迁移到插件系统（repeat_plugin），不再需要手动调用
            # 旧的复读检测器代码已禁用
            # if chat.group_info and message.processed_plain_text:
            #     try:
            #         from src.proactive_system.repeat_detector import get_repeat_detector
            #         repeat_detector = get_repeat_detector()
            #         asyncio.create_task(
            #             repeat_detector.check_and_repeat(
            #                 stream_id=chat.stream_id,
            #                 current_message_text=message.processed_plain_text
            #             )
            #         )
            #     except Exception as e:
            #         logger.error(f"复读检测失败: {e}")
