...
class DefaultReplyer:
    def __init__(
        ...
        self.heart_fc_sender = get_universal_message_sender()
    ...
    async def build_relation_info(self, sender: str):
        """构建关系信息
        ...
        if not person.is_known:
            logger.debug(f"未找到用户 {sender} 的信息，跳过关系信息构建")
            return ""

        try:
            # 获取关系信息
            relation_info = await person.build_relationship("")
            if relation_info:
                return f"\n**你与 {sender} 的关系：**\n{relation_info}"
            return ""
        except Exception as e:
...
"""
    ...
    async def build_personality_prompt(self) -> str:
        ...
        # 添加功能信息，让 MaiBot 知道自己有哪些功能
        capabilities_info = await self._build_capabilities_info()

        return f"你的名字是{bot_name}{bot_nickname}，你{prompt_personality}\n{capabilities_info}"

    async def _build_capabilities_info(self) -> str:
        """构建功能信息，让 MaiBot 知道自己有哪些已启用的功能"""
        try:
            ...
            user_actions = {name: info for name, info in enabled_actions.items()
                          if name not in ["emoji", "build_memory", "build_relation", "reply"]}
            ...
            if enabled_plugins:
                ...
                capabilities_parts.append(f"你已启用的插件有：{', '.join(plugin_names)}。")

            if user_actions:
                action_names = list(user_actions.keys())
                capabilities_parts.append(f"你可以执行的动作有：{', '.join(action_names)}。")

            if enabled_commands:
                command_names = list(enabled_commands.keys())
                capabilities_parts.append(f"你可以使用的命令有：{', '.join(command_names)}。")

            if enabled_tools:
                tool_names = list(enabled_tools.keys())
                capabilities_parts.append(f"你可以使用的工具有：{', '.join(tool_names)}。")

            if capabilities_parts:
                return " ".join(capabilities_parts)
            else:
                return "你当前没有启用任何额外功能。"

        except Exception as e:
            logger.error(f"构建功能信息失败: {e}", exc_info=True)
            return "你当前没有启用任何额外功能。"
    ...
    async def build_prompt_reply_context(
        ...
        # 场景A：添加跨场景记忆（在群聊中记得私聊内容）
        cross_scene_memory = ""
        enable_cross_scene_memory = global_config.chat.enable_cross_scene_memory if hasattr(global_config.chat, 'enable_cross_scene_memory') else True
        if enable_cross_scene_memory and _is_group_chat and reply_message:
            # 只在群聊中且有明确的回复对象时添加跨场景记忆
            try:
                # 获取发言者的跨场景记忆点
                person = Person(platform=platform, user_id=user_id)
                if person.is_known and person.memory_points:
                    memory_points = person.memory_points[:5]  # 最多显示5条
                    if memory_points:
                        cross_scene_memory = f"\n你对 {person_name} 的了解（来自其他场景的记忆）：\n"
                        for memory in memory_points:
                            ...
                            if len(parts) >= 2:
                                category = parts[0]
                                content = parts[1]
                                cross_scene_memory += f"- {category}：{content}\n"
                            else:
                                cross_scene_memory += f"- {memory}\n"
                        logger.debug(f"添加了 {person_name} 的跨场景记忆到群聊回复中")
            except Exception as e:
                logger.error(f"获取跨场景记忆失败: {str(e)}")
                cross_scene_memory = ""

        # 增强记忆：添加详细的用户记忆摘要
        detailed_memory = ""
        enable_detailed_memory = global_config.memory.enable_detailed_memory if hasattr(global_config.memory, 'enable_detailed_memory') else True
        if enable_detailed_memory and reply_message:
            try:
                from src.memory_system.user_memory_summary import get_user_memory_for_reply
                person = Person(platform=platform, user_id=user_id)
                if person.is_known:
                    detailed_memory = await get_user_memory_for_reply(
                        person_id=person.person_id,
                        chat_id=chat_id,
                        enable_detailed_memory=enable_detailed_memory,
                    )
                    if detailed_memory:
                        logger.debug(f"添加了 {person_name} 的详细记忆摘要")
            except Exception as e:
                logger.error(f"获取详细记忆摘要失败: {str(e)}")
                detailed_memory = ""

        # 将跨场景记忆和详细记忆添加到记忆检索块中
        if cross_scene_memory or detailed_memory:
            additional_memory = cross_scene_memory + detailed_memory
            memory_retrieval = memory_retrieval + additional_memory if memory_retrieval else additional_memory
