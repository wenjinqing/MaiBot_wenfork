...
# 多线程embedding配置常量
DEFAULT_MAX_WORKERS = 3  # 默认最大线程数（降低以避免速率限制）
DEFAULT_CHUNK_SIZE = 5  # 默认每个线程处理的数据块大小（降低以避免速率限制）
MIN_CHUNK_SIZE = 1  # 最小分块大小
MAX_CHUNK_SIZE = 10  # 最大分块大小（降低以避免速率限制）
MIN_WORKERS = 1  # 最小线程数
MAX_WORKERS = 5  # 最大线程数（降低以避免速率限制）
