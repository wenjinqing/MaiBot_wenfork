...
class BrainChatting:
    ...
    async def _send_response(
        ...
        should_mention = MentionHelper.should_mention_user(
            ...
            is_proactive=False,
            is_group=is_group,
            recent_message_count=new_message_count,
        )
        ...
        for reply_content in reply_set.reply_data:
            ...
            # 在第一条消息中添加 @ （如果需要）
            if not first_replied and should_mention:
                mention_target = MentionHelper.get_mention_target(message_data)
                if mention_target:
                    user_id, user_nickname = mention_target
                    data = MentionHelper.add_mention_to_text(data, user_id, user_nickname)
