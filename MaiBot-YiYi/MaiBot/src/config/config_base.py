...
class ConfigBase:
    ...
    def _convert_field(cls, value: Any, field_type: Type[Any]) -> Any:
        ...
        if field_origin_type is Union:
            # Optional[X] 实际上是 Union[X, None]
            # 过滤掉 None 类型，获取实际类型
            non_none_types = [t for t in field_type_args if t is not type(None)]
            if len(non_none_types) == 1:
                # 这是 Optional[X] 的情况
                actual_type = non_none_types[0]
                return cls._convert_field(value, actual_type)
            else:
                # 这是真正的 Union 类型，尝试每个类型
                for t in non_none_types:
                    try:
                        return cls._convert_field(value, t)
                    except (TypeError, ValueError):
                        continue
                raise TypeError(f"Cannot convert value to any of the Union types: {field_type_args}")
