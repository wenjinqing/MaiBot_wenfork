...
class Person:
    ...
    async def build_relationship(self, chat_content: str = "", info_type=""):
        ...
        # 检查恋爱状态
        love_status_str = ""
        relationship_level_str = ""
        try:
            from src.common.database.database_model import PersonInfo, db
            with db:
                person_record = PersonInfo.get_or_none(
                    (PersonInfo.user_id == self.user_id) &
                    (PersonInfo.platform == self.platform)
                )
                if person_record:
                    # 获取关系等级和好感度
                    relationship_value = person_record.relationship_value
                    if relationship_value >= 90:
                        relationship_level_str = f"（关系等级：挚友，好感度：{relationship_value:.1f}/100）"
                    elif relationship_value >= 70:
                        relationship_level_str = f"（关系等级：好友，好感度：{relationship_value:.1f}/100）"
                    elif relationship_value >= 50:
                        relationship_level_str = f"（关系等级：熟人，好感度：{relationship_value:.1f}/100）"
                    elif relationship_value >= 30:
                        relationship_level_str = f"（关系等级：认识，好感度：{relationship_value:.1f}/100）"
                    else:
                        relationship_level_str = f"（关系等级：陌生，好感度：{relationship_value:.1f}/100）"

                    # 检查恋爱状态
                    if person_record.is_in_love:
                        # 计算在一起的天数
                        if person_record.anniversary_date:
                            days_together = int((time.time() - person_record.anniversary_date) / 86400)
                            if days_together == 0:
                                love_status_str = f"💕 重要：{self.person_name} 是你的恋人（今天刚在一起）"
                            else:
                                love_status_str = f"💕 重要：{self.person_name} 是你的恋人（已经在一起 {days_together} 天了）"
                        else:
                            love_status_str = f"💕 重要：{self.person_name} 是你的恋人"
        except Exception as e:
            logger.error(f"获取恋爱状态失败: {e}")
        ...
        if self.memory_points:
            ...
            for mp in self.memory_points:
                if not mp or not isinstance(mp, str):
                    continue
                cat = get_category_from_memory(mp)
                content = get_memory_content_from_memory(mp)
                if not content:
                    content = mp
                if cat:
                    categorized.setdefault(cat, []).append(content)
                else:
                    categorized.setdefault("其他", []).append(content)

            if categorized:
                lines = []
                for cat, contents in categorized.items():
                    lines.append(f"[{cat}] " + "；".join(contents))
                points_text = "；\n".join(lines)

        points_info = ""
        if points_text:
            points_info = f"\n- 你还记得有关{self.person_name}的内容：{points_text}"

        # 构建完整的关系信息
        if love_status_str:
            # 恋人关系，最优先显示
            relation_info = f"{love_status_str}"
            if nickname_str:
                relation_info += f"\n- 昵称：{nickname_str}"
            if points_info:
                relation_info += points_info
        elif relationship_level_str or nickname_str or points_info:
            # 普通关系
            relation_info = f"{self.person_name} {relationship_level_str}"
            if nickname_str:
                relation_info += f"\n- 昵称：{nickname_str}"
            if points_info:
                relation_info += points_info
